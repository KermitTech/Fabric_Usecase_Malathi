...
# Get latest formatted value
latest_row = df.orderBy(df["CurrentRunTime"].desc()).select("FormattedRunTime").limit(1).collect()

# Extract value or return null
previous_run_time = latest_row[0]["FormattedRunTime"] if latest_row else None
